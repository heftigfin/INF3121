...
public class MineField{
	...
	//checks if the row/col has a mine.
	...
	//goes thru the board and makes all the mines visible.
	private void boom() {
		for(int row=0;row<rowMax;row++){
			for(int col=0;col<colMax;col++){
				if(mines[row][col]){
	...
	//prints the board. Where a mine is found with boom() a "*" is printed and "-" where there was no mine. Untried spaces results in "?".
	...
	// It separates the string and uses only the 2 first chars which must be split by a " "
	// If it doesn throw an Exception it calls on legalMoveValue()
	...
	//It checks for 2 things, if you've already chosen row/col ones before. Or if there is a mine there.
